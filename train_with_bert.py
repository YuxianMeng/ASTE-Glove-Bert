...
from bert_data_utils import ABSADatasetReader
from bert_models import BERT_GCN
from evaluation_bert import get_metric, find_pair, find_term,  compute_sentiment, find_pair_sentiment, find_grid_term
from utils import *
...
class Instructor:
    def __init__(self, opt):
        ...
        self.f_out = open('log/'+self.opt.model_name+'_'+self.opt.dataset+'_val'+str(now_time)+'.txt', 'w', encoding='utf-8')
        self._print_args()
        self.global_f1 = 0.
    ...
    def _train(self, criterion, optimizer):
        ...
        for epoch in (range(self.opt.num_epoch)):
            ...
            for i_batch, sample_batched in enumerate(self.train_data_loader):
                ...
                if self.opt.if_lambda == 1:
                    if i_batch == 0:
                        lambda_ = i_batch * (1 / (len(self.absa_dataset.train_data)/self.opt.batch_size))
                    else:
                        lambda_ = 1
                else:
                    lambda_ = 1
                # loss = loss_aspect + loss_opinion + lambda_*loss_pair + lambda_*loss_pair_sentiment
    ...
    def run(self):
        ...
        for i in range(self.opt.repeats):
            ...
            optimizer = torch.optim.Adam([
                {'params': self.model.bert_model.parameters(), 'lr': self.opt.learning_rate},
...
if __name__ == '__main__':
    ...
    input_colses = {
        ...
        # 'lstm': ['text_indices', 'mask', 'local_graph', 'global_graph', 'relevant_sentences'],
        # 'ascnn': ['text_indices', 'mask', 'text_graph'],
        # 'asgcn': ['text_indices', 'mask', 'text_graph'],
        # 'astcn': ['text_indices', 'mask', 'text_graph'],
        # 'min': ['text_indices', 'mask', 'local_graph', 'global_graph', 'relevant_sentences', 'relevant_sentences_presentation', \
        #     'pair_labels', 'sentiment_labels', 'local_graph_pmi', 'aspect_sequence_labels','opinion_sequence_labels','sentiment_sequence_labels'], \
        # 'seq': ['text_indices', 'mask', 'local_graph', 'global_graph', 'relevant_sentences', 'relevant_sentences_presentation', \
        #     'pair_labels', 'sentiment_labels', 'local_graph_pmi', 'aspect_sequence_labels','opinion_sequence_labels','sentiment_sequence_labels'], \
        # 'ts': ['text_indices', 'mask', 'aspect_sequence_labels','opinion_sequence_labels','sentiment_sequence_labels'],\
        # 'ts_focus': ['text_indices', 'mask', 'aspect_sequence_labels','opinion_sequence_labels','sentiment_sequence_labels'],\
        ...
        # 'ts6': ['text_indices', 'mask', 'global_graph0', 'relevant_sentences', 'relevant_sentences_presentation', \
        #     'pair_grid_labels', 'triple_grid_labels', 'aspect_sequence_labels','opinion_sequence_labels','sentiment_sequence_labels'],\
        # 'ts7': ['text_indices', 'mask', 'global_graph0', 'relevant_sentences', 'relevant_sentences_presentation', \
        #     'pair_grid_labels', 'triple_grid_labels', 'aspect_sequence_labels','opinion_sequence_labels','sentiment_sequence_labels'],\
        # 'ts0_': ['text_indices', 'mask', 'local_graph', 'global_graph', 'relevant_sentences', 'relevant_sentences_presentation', \
        #     'pair_labels', 'sentiment_labels', 'local_graph_pmi', 'aspect_sequence_labels','opinion_sequence_labels','sentiment_sequence_labels'],\
        ...
        #     'pair_grid_labels', 'triple_grid_labels',  'aspect_sequence_labels','opinion_sequence_labels','sentiment_sequence_labels'],\
        # 'ts2_': ['text_indices', 'mask', 'global_graph0', 'global_graph1', 'global_graph2', 'relevant_sentences', 'relevant_sentences_presentation', \
        #     'pair_grid_labels', 'triple_grid_labels',  'aspect_sequence_labels','opinion_sequence_labels','sentiment_sequence_labels'],\
        # 'ts3': ['text_indices', 'mask', 'global_graph0', 'global_graph1', 'global_graph3', 'relevant_sentences', 'relevant_sentences_presentation', \
        #     'pair_grid_labels', 'triple_grid_labels', 'aspect_sequence_labels','opinion_sequence_labels','sentiment_sequence_labels'],\
        # 'ts4': ['text_indices', 'mask', 'local_graph', 'global_graph', 'global_graph1', 'global_graph2', 'global_graph3', 'relevant_sentences', 'relevant_sentences_presentation']
    }
    initializers = {
