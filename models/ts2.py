...
#         self.fc_aspect_h = nn.Linear(600, 300)
#         self.fc_opinion_h = nn.Linear(600, 300)
#         self.fc_sentiment_h = nn.Linear(600, 300)
#         self.fc_pair_h = nn.Linear(1200, 600)
#         self.fc_pair_sentiment_h = nn.Linear(1200, 600)
...
class TS2(nn.Module):
    def __init__(self, embedding_matrix, opt):
        ...
        self.gru = DynamicLSTM(300, 300, num_layers=1, batch_first=True, bidirectional=True, rnn_type = 'GRU')
        self.gru_ = DynamicLSTM(300, 300, num_layers=1, batch_first=True, bidirectional=True, rnn_type = 'GRU')
        self.dilstm = DynamicLSTM(300, 300, num_layers=1, batch_first=True, bidirectional=False, rnn_type = 'LSTM')
        ...
        # private single
        self.fc_aspect = nn.Linear(900, 3)
        self.fc_opinion = nn.Linear(900, 3)
        self.fc_pair = nn.Linear(1800, 3)
        self.fc_pair_sentiment = nn.Linear(1800, 4)
        # shared private 
    ...
    def forward(self, inputs, mask):     
        ...
        if self.opt.use_aspect_opinion_sequence_mask == 1:   
            ...
            if self.opt.emb_for_ps == 'private_single':
                ...
                pair_probs = pair_probs_.contiguous().view(-1, 3)
                pair_sentiment_probs = self.fc_pair_sentiment(pair_text.float()).contiguous().view(-1, 4)
            elif self.opt.emb_for_ps == 'shared_multi':
